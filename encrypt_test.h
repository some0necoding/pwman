...
int write_cypher_to_file(unsigned char *header, unsigned char *cypher_text, char *file_path);

/*-----------FUNCTIONS-DEFINITION-END-----------*/


// the name talks by itself, anyway this function encrypts the file located at "file_path"
// using the key "key". The key point here is that all the encryption process is done in memory
// using buffers, so that is only used a single file and not an encrypted file and a decrypted file.
// In the reality we'll still have a plain text file to encrypt that is stored in the filesystem and 
// we don't like this kinda things. It'd be better to handle plain text only in memory for security
// purposes (encryption: plain buffer -> encrypted file; decryption: encrypted file -> plain buffer).
// The probabilities that the plain text get stolen before encryption or that some scratches of the file 
// remain in the disk (a lot of forensic procedures exist to retrieve deleted or hidden files from disks) 
// are low, but still possibilities, instead memory is easier cleanable.
int encrypt_file(char *file_path, unsigned char *key)
{
    ...
    printf("good encryption\n");
    
    // write the encrypted buff into the file
    if (write_cypher_to_file(header, crptd_text, file_path) != 0) {
        perror("psm: I/O error\n");
        return -1;
    }

    printf("good file writing\n");
...
int write_cypher_to_file(unsigned char *header, unsigned char *cypher_text, char *file_path)
{
    ...
    size_t cypher_len = sizeof(cypher_text);
    ...
    if ((rlen = fwrite(cypher_text, 1, cypher_len, file)) != cypher_len) {
